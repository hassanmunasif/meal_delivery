...
class Order:
    ...
    """
    instance_count = 0
    instant_order_count = 0
    pre_order_count = 0

    def __init__(self, customer_id, start_at, estimated_preparation_time,
                 actual_preparation_time, order_type):
        self.order_type = 'instant' if random.random() < 0.7 else 'pre_order'
        self.customer_id = customer_id
        self.start_at = start_at
        self.estimated_preparation_time = estimated_preparation_time
        self.actual_preparation_time = actual_preparation_time
        Order.instance_count += 1

        self.get_order_type()

    def get_order_type(self):
        if self.order_type == 'pre_order':
            Order.pre_order_count += 1
        elif self.order_type == 'instant':
            Order.instant_order_count += 1
